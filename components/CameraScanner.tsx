...
import { Stack, Button, Alert, Text, Select, Box, Center, ThemeIcon, Loader } from "@mantine/core";
    ...
    useEffect(() => {
        ...
        Html5Qrcode.getCameras()
            .then((devices) => {
                if (devices && devices.length > 0) {
                    const formattedDevices = devices.map((dev) => ({
                        id: dev.id,
                        label: dev.label || `Camera ${devices.indexOf(dev) + 1}`,
                    }));
            ...
            .catch((err) => {
                ...
                setError("Missing camera permissions or camera not supported.");
            });
        ...
        <Stack gap="md">
            ...
            >
                ...
                <div
                    ...
                    style={{
                        ...
                        height: "100%",
                        minHeight: "300px",
                        position: (status === "scanning" || status === "stopping") ? "relative" : "absolute",
                        opacity: (status === "scanning" || status === "stopping") ? 1 : 0,
                        zIndex: (status === "scanning" || status === "stopping") ? 1 : -1,
                        pointerEvents: (status === "scanning" || status === "stopping") ? "auto" : "none"
                    }}
                />
                ...
                {status === "stopping" && (
                    <Center h={300} pos="absolute" top={0} left={0} w="100%" style={{ backgroundColor: "rgba(0,0,0,0.5)", zIndex: 10 }}>
                        <Stack align="center" gap="xs">
                            <Loader size="sm" color="white" />
                            <Text c="white">Processing scan...</Text>
                        </Stack>
                    </Center>
                )}
            </Box>
            ...
            <Center>
                <Button
                    ...
                    leftSection={status === "scanning" ? <IconAlertCircle size={18} /> : <IconCamera size={18} />}
                    size="lg"
                >
